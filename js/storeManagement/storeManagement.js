(function(document, window, $) {
    ...
    "click #delete": function(e, v, row) {
      firm("提示", "是否删除", function() {
        ...
        ajax_data(
          ...
          { params: { storeId: row.storeId}, type: "post" },
          function(res) {
            console.log(res);
            if (res.resCode > 0) {
              tips("删除成功", 6);
  ...
  function queryParams(params) {
    return {
      provinceId:$(".params_province").val() ? $(".params_province").val() : undefined,
      cityId:$(".params_city").val() ? $(".params_city").val() :undefined,
      areaId:$(".params_area").val() ? $(".params_area").val() : undefined,
      storeName:$(".query_StoreName").val()? $(".query_StoreName").val():undefined
    };
  }
  initFn();
  // 点击查询按钮
  ...
  function queryCity(dom,params) {
    ...
    ajax_data("/configuration/queryCity", { params: JSON.stringify(params),async:false }, function(res) {
  ...
  function queryCounty(dom,params) {
    ajax_data("/configuration/queryArea", { params: JSON.stringify(params),async:false}, function(res) {
      var option='<option value="">选择省份</option>';
      for(let i in res){
        option+=`<option value="${res[i].id}">${res[i].name}</option>`
      }
  ...
  $(".params_province").change(function() {
    if($(this).val() == ""){
      $(".params_city,.params_county").val("");
      $(".params_city,.params_county").attr("disabled",true);
    }else{
      $(".params_city,.params_county").attr("disabled",false);
    }
