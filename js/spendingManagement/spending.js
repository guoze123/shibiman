...
(function (document, window, $) {
    ...
    $("#spending").bootstrapTable({
      ...
      // },
      {
        ...
        formatter: function formatter(value, row) {
          return "<img  class=\"viewImg\" src=\"".concat(base + "/uploadImgs/" + row.costId + ".jpg", "\"  style=\"width:50px;height:50px\">");
        },
        events: {
          "click .viewImg": function clickViewImg(e, v, row) {
            var url = base + "/uploadImgs/" + row.costId + ".jpg";
            var image = new Image();
            image.src = url;
  ...
  var operateEvents = {
    "click #edit": function clickEdit(e, v, row) {
      ...
      $(".remark").attr("data_costId", row.costId); // $("#editData img").attr("src",base+"/uploadImgs/"+row.costId+".jpg")
  ...
  initFn();
  document.getElementById("spending").addEventListener("error", function (event) {
  ...
  function queryDepartment() {
    ajax_data("/competence/queryDepartment", {
      params: JSON.stringify({})
    }, function (res) {
      var option = "<option value=''>选择部门</option>";
      res.forEach(function (element) {
        option += "<option value=\"".concat(element.departmentId, "\">").concat(element.departmentName, "</option>");
      });
      $(".ownerId").html(option);
    });
  } // 开支分类
  ...
  function queryCostType() {
    ...
    ajax_data("/cost/queryCostCategory", {
      params: JSON.stringify(params),
      async: false
    }, function (res) {
      var option = "<option value=''>开支分类</option>";
      res.forEach(function (element) {
        option += "<option value=\"".concat(element.categoryId, "\">").concat(element.categoryName, "</option>");
      });
  ...
  $(".exportBtn").click(function () {
    ...
    $("<input>").attr("name", "type").val("1").appendTo(form);
