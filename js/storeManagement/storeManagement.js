...
(function (document, window, $) {
  ...
  function initFn() {
    down_list(".queryStoreManager", "/personnel/queryEmployeeInfo", "选择店长", "telephone");
    $("#exampleTableFromData").bootstrapTable({
      method: "post",
      url: base + "/competence/queryStoreInfo",
      //请求路径
      striped: true,
      //是否显示行间隔色
      pageNumber: 1,
      ...
      columns: [{
        ...
        title: "店铺类型",
        field: "storeType",
        formatter: function formatter(vlaue, row) {
  ...
  var operateEvents = {
    ...
    "click #delete": function clickDelete(e, v, row) {
      firm("提示", "是否删除", function () {
        layer.closeAll("page");
        ajax_data("/competence/deleteStoreInfo", {
          params: {
            storeId: row.storeId
          },
  ...
  function queryParams() {
    return {
      provinceId: $(".params_province").val().trim() ? $(".params_province").val().trim() : undefined,
  ...
  function confirmFn() {
    ...
    if ($(".params_province").val() == "" || $(".params_city").val() == "" || $(".params_area").val() == "") {
      tips(requiredText, 5);
      return;
    }
  ...
  function queryProvince() {
    ajax_data("/configuration/queryProvince", {
      params: {}
    }, function (res) {
      var option = '<option value="">选择省份</option>';

      for (var i in res) {
        option += "<option value=\"".concat(res[i].id, "\">").concat(res[i].name, "</option>");
      }
  ...
  function queryCity(dom, params) {
    ajax_data("/configuration/queryCity", {
      params: params,
      async: false,
      ...
      $(dom).change(function () {
        if ($(this).val().trim() == "") {
          $(this).next().html('<option value="">选择区县</option>');
        }

        var newParams = params;
        newParams["cityId"] = parseInt($(this).val().trim());
  ...
  function queryCounty(dom, params) {
    ajax_data("/configuration/queryArea", {
      params: params,
      async: false,
      contentType: "application/x-www-form-urlencoded"
    }, function (res) {
  ...
  $(".query_province").change(function () {
    ...
    queryCity(".query_city", {
      provinceId: parseInt($(this).val().trim())
    });
  });
